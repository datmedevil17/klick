...
export function KeyboardScene({ settings, onBackToMenu }: KeyboardSceneProps) {
  // Typing state
  ...
  // New word set function
  const newWordSet = useCallback(() => {
    setWords(generateRandomWords(settings.difficulty, settings.wordCount))
    resetSession()
  ...
  const handleWordComplete = useCallback((word: string, isCorrect: boolean) => {
    ...
    if (isCorrect) {
      ...
      setCorrectCharacters(prev => prev + word.length + 1) // +1 for space
    } else {
      setIncorrectWords(prev => prev + 1)
    }
    
    setTotalCharacters(prev => prev + word.length + 1) // +1 for space
  ...
  }, [capsLock, shiftPressed, typedText, words, currentWordIndex, hasStarted, isFinished, handleWordComplete])
